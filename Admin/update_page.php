...
if(!isset($_POST['ParentPage']) OR $_POST['ParentPage']==""){
	$_POST['ParentPage'] = NULL;
}
...
if(!isset($_POST['menuOrder']) OR $_POST['menuOrder']==""){
	$_POST['menuOrder'] = NULL;
}
...
$query = "UPDATE Pages SET Title=?, Content=?, ParentPage=?, Slug=?, Template=?, Content2=?, GalleryName=?, pubDate=?, IncludeInNav=?, SqlDate=?, MenuOrder=?, pageType=? WHERE page_id=?";
...
$stmt->bindParam(13, $_POST["page_id"]);
...
if ($stmt->execute() == TRUE) {
  ...
  if ($_POST["pageType"] == 'Page'){
   echo '<form action="page_create_edit.php" method="post">';
      ...
      echo '<form action="blog_create_edit.php" method="post">';
   }
   echo'
